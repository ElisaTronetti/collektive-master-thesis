...
Metaprograms \cite{metaprogramming_introduction} are able to take as input another program, sometimes even the metaprogram itself, manipulate it and return a new version of it expressing a modified behavior.\newline 
...
KSP has been developed as an alternative technology to \textbf{kapt} \cite{kapt_documentation}, which was used to allow annotation processing. Currently, kapt is in maintenance mode, meaning that it is being kept up-to-date with the new Kotlin and Java releases, but without adding any features.\newline
...
The complexities involved in Kotlin compiler plugins, discussed in the previous sections, need a further exploration. For this reason, it has been developed a simple plugin \cite{km_compiler_plugin_example}, which aims to dive deeply into the details of basic and advanced features implementation.
